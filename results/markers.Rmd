...
write_tsv(naive_atcells_markers, file.path("markers", "naive_cell_types_markers_wilcox.txt"))
```
...
save_plot(file.path(outdir, "td_markers.pdf"), 
          p, 
          base_width = 6, 
          base_height = 6)
```
...
```{r dotplot_notch_pops, fig.height = 6, fig.width = 6}
atcells <- SetAllIdent(atcells, "labeled_clusters")
...
marker_dat <- FetchData(atcells, notch_markers$notch)
marker_dat <- factor(unique(unlist(notch_markers)))[order(colMaxs(marker_dat),decreasing = T)]
...
save_plot(file.path(outdir, "notch_markers.pdf"), 
          p, 
          base_width = 6, 
          base_height = 6)
```
