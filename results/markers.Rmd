...
```{r id_tsne1, fig.width = 14, fig.height = 10, fig.cap= "TSNE plot with each cell colored by the cluster id"}
atcells <- SetAllIdent(atcells, "res.0.6")
...
inj_cells <- rownames(atcells@meta.data)[atcells@meta.data$res.0.6 %in%
                                           inj_clusters]

tmp_dat <- SubsetData(atcells, cells.use = inj_cells)
tmp_dat <- SetAllIdent(tmp_dat, "res.0.6")
...
### All injured against naive type II
...
inj_cells <- rownames(atcells@meta.data)[atcells@meta.data$res.0.6 %in%
                                           inj_clusters]
naive_type_ii <- rownames(atcells@meta.data)[atcells@meta.data$labeled_clusters == "Naive Type II"]

selected_cells <- c(inj_cells, naive_type_ii)

tmp_dat <- SubsetData(atcells, cells.use = selected_cells)
tmp_dat <- SetAllIdent(tmp_dat, "res.0.6")


inj_vs_naive_cluster_markers <- FindAllMarkers(tmp_dat, 
                                  only.pos = TRUE,  
                                  min.pct = 0.25, 
                                  thresh.use = 0.25)

names(inj_vs_naive_cluster_markers) <- paste0(inj_clusters,
                                              "_vs_naive_AEC2")

inj_vs_naive_cluster_markers <- map(inj_vs_naive_cluster_markers, 
                                    as_tibble, 
                                    rownames = "gene")

mks_df <- bind_rows(inj_vs_naive_cluster_markers, .id = "comparison")

write_tsv(mks_df,
          file.path(outdir,
            "injured_cluster_vs_naive_t2_markers_wilcox.txt"))
rm(tmp_dat)
gc()
```
...
all_cell_type_markers <- FindAllMarkers(SetAllIdent(atcells, "pretty_cell_labels"),
                                  only.pos = TRUE,  
                                  min.pct = 0.25, 
                                  thresh.use = 0.25)

write_tsv(all_cell_type_markers,
          file.path(outdir,
            "all_labled_cell_markers_wilcox.txt"))
```

### Naive I/II and td pops
...
query_cell_ids <- rownames(atcells@meta.data)[atcells@meta.data$labeled_clusters %in% query_cell_types]

atcells_subset <- SubsetData(atcells, cells.use = query_cell_ids)

query_cell_type_markers <- FindAllMarkers(SetAllIdent(atcells_subset, "pretty_cell_labels"),
                                  only.pos = TRUE,  
                                  min.pct = 0.25, 
                                  thresh.use = 0.25)

write_tsv(query_cell_type_markers,
          file.path(outdir,
...
```{r all_clusters}
markers <- read_tsv(file.path(outdir, "all_cluster_markers_wilcox.txt"))
...
```{r all_clusters_labeled}
markers <- read_tsv(file.path(outdir, "all_labled_cell_markers_wilcox.txt"))
list_of_markers <- split(markers, markers$cluster)
list_of_markers <- map(list_of_markers, 
                       ~set_xlsx_class(.x, "gene", "Text"))
...
readme_sheet <- list(README = readme_sheet)
names(readme_sheet) <- "README"
names(list_of_markers) <- str_replace(names(list_of_markers), ":", "")  %>% str_replace("/", "_")
openxlsx::write.xlsx(c(readme_sheet, 
                       list_of_markers), 
                     file.path("markers",
                               "markers_per_labeled_cluster.xlsx"))

```

```{r selected_clusters_labeled}
markers <- read_tsv(file.path(outdir, "td_pops_and_naive_markers_wilcox.txt"))
list_of_markers <- split(markers, markers$cluster)
list_of_markers <- map(list_of_markers, 
                       ~set_xlsx_class(.x, "gene", "Text"))
...
readme_sheet <- list(README = readme_sheet)
names(readme_sheet) <- "README"
names(list_of_markers) <- str_replace(names(list_of_markers), ":", " ")
openxlsx::write.xlsx(c(readme_sheet, 
                       list_of_markers), 
                     file.path("markers",
                            "markers_per_td_pops_and_naive_clusters.xlsx"))

```


```{r all_injured_clusters}
markers <- read_tsv(file.path(outdir, "injured_cluster_markers_wilcox.txt"))
list_of_markers <- split(markers, markers$cluster)
list_of_markers <- map(list_of_markers, 
                       ~set_xlsx_class(.x, "gene", "Text"))
...
readme_sheet <- data_frame(
  Columns = c(
  "Genes differentially expressed between each injured AEC2 cluster and all other injured AEC2 cells",
  "",
  "Columns",
  "pval",
  "avg_logFC",
...
openxlsx::write.xlsx(c(readme_sheet, 
                       list_of_markers), 
                     file.path("markers",
                               "markers_per_injured_cluster.xlsx"))
...
markers <- split(markers, markers$comparison)
...
openxlsx::write.xlsx(c(readme_sheet, markers),
                     file.path("markers","markers_per_naive_typeii_vs_each_injured_cluster.xlsx"))
...
readme_sheet <- data_frame(
  Columns = c(
  "Genes differentially expressed between injured AEC2 clusters and Naive AEC2 cluster",
  "",
  "Columns",
  "comparison",
  "gene",
  ...
  "",
  "",
  "clusters tested against naive AEC2",
  "gene name",
  "p-value from wilcox test of indicated cluster compared to other clusters",
  "average fold change expressed in natural log",
  "percent of cells expressing gene (UMI > 0) in cluster",
...
### Find Transcription Factors {.tabset .tabset-fade}
 ...
 Tfs were selected by being called TFS by either the TF checkpoint database of the DBD database (columns DbTF == "yes" or DBD == "DBD")
...
annotated_tfs <- left_join(tfs, tbl_df(eid_to_symbol), 
  ...
  dplyr::filter(entrez_mouse != 0,  #0 is na 
                DBD == "DBD" | DbTF == "yes")  #remove annotations like BMP4
...
openxlsx::write.xlsx(c(readme_sheet, per_cluster), 
                     file.path("markers",                                  "transcription_factors_per_td_injured_clusters_01252019.xlsx"))
```
...
# each element of named lists will become a seperate sheet
openxlsx::write.xlsx(c(readme_sheet, per_cluster), 
                     file.path("markers",                                  "transcription_factors_per_td_injured_clusters_downregulated_012519.xlsx"))
