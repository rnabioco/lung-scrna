...
source("../R/globals.R")
processed_data_dir <- file.path(results_dir, 
                                "preprocess")
...
save_plot(file.path(f1_outdir, "1A_tsne_no_labels.pdf"), plt,
...
save_plot(file.path(f1_outdir, "1E_tsne_naive_nonepi.pdf"), plt,
...
s5_outdir <- file.path(main_outdir, "supplement", "fig5")
dir.create(s4_outdir, recursive = T, showWarnings = F)
dir.create(s5_outdir, recursive = T, showWarnings = F)
...
# top 3 genes only
tsne_plts <- pmap(list(cell_type_markers[c("AEC1", "AEC2")],
                       names(cell_type_markers[c("AEC1", "AEC2")]),
                       c("C", "D")),
                  function(x, y, panel) {
                    plts <- list()
                    plts[[1]] <- tsne_by_sample_type(
                      atcells,
                      .gene = x[1],
                      cell_sets = naive_cell_types
                    )
                    
                    plts_no_label <- map(x[2:min(3, length(x))],
                                         ~ tsne_by_sample_type(atcells,
                                                               .gene = .x,
                                                               cell_sets = naive_cell_types))
                    all_plts <- c(plts, plts_no_label)
                    plt <- plot_grid(plotlist = all_plts, 
                                     ncol = 1)
                    
                    save_plot(
                      file.path(f1_outdir,
                                paste0("1", panel, "_tsne_", y, ".pdf")),
                      plt,
                      ncol = 1,
                      nrow = length(all_plts),
                      base_height = 2.75,
                      base_aspect_ratio = 3
                    )
                    plt
...
tsne_plts <- pmap(list(cell_type_markers[c("AEC1", "AEC2")],
                  ...
                  function(x, y, panel) {
                    ...
                    plts_no_label <- map(x[5:length(x)],
                                         ~ tsne_by_sample_type(atcells,
                                                               .gene = .x,
                                                               cell_sets = naive_cell_types))
                    all_plts <- c(plts, plts_no_label)
...
save_plot(file.path(f1_outdir, "1B_tsne_per_sampletype.pdf"), plt, 
...
p <- DotPlot(naive_atcells, 
  ...
  scale_y_discrete(labels= str_remove(levels(naive_atcells@ident), "Naive ")) +
  coord_flip() + 
  labs(colour = "Z-Score\n(Average\nExpression)",
...
save_plot(file.path(f1_outdir, "1F_canonical_markers_dotplot.pdf"), 
          p, 
          base_width = 6, 
          base_height = 5)
```
...
tmp_mat <- t(mat)
rownames(tmp_mat) <- str_remove(rownames(tmp_mat), "Naive ")

pdf(file.path(f1_outdir, "1G_naive_top_markers_viridiscols.pdf"), 
    width = 14, height = 4)

Heatmap(tmp_mat, col = viridis(9),
        name = "Expression\n(Z-score)",
        cluster_rows = F,
        cluster_columns = F,
        show_row_dend = F,
        show_column_dend = F,
        show_column_names = T, 
        row_names_side = "left", column_names_gp = gpar(fontface = "italic"))   
dev.off()
...
plt <- plot_feature(atcells, feature = "res.0.6", 
                 label_text = T, 
                 label_color = "Black", label_size = 6,
                 .cols = many_cols) + 
  theme(legend.position = "none")

save_plot(file.path(s4_outdir, "S4C_tsne_cluster_ids.pdf"), 
          plt, base_height = 4, base_aspect_ratio = 1.0)

plt <- plot_feature(atcells, feature = "res.0.6", 
                 .cols = many_cols) 

save_plot(file.path(s4_outdir, "S4C_tsne_cluster_ids_with_legend.pdf"), 
          plt, base_height = 4, base_aspect_ratio = 1.2)

plt <- plot_feature(atcells, feature = "res.0.6", 
                 .cols = many_cols) + 
  theme(legend.position = "none")

save_plot(file.path(s4_outdir, "S4C_tsne_cluster_ids_without_labels.pdf"), 
          plt, base_height = 4, base_aspect_ratio = 1.0)

plt <- plot_feature(atcells, feature = "res.0.6", 
                 label_text = F,  
                 .cols = many_cols) 
save_plot(file.path(f2_outdir, "2F_tsne_cluster_ids_no_labels.pdf"), 
...
### without labels
map2(supplement_cell_markers,
  names(supplement_cell_markers),
    function(x, y){
  plts <- list()
  exp_limit = NULL
...
plts <- map(other_cca ,
    ~plot_feature(atcells, feature = .x, 
                  cell_filter = naive_injured_cell_types$`Injured AT-II Derived`))

plt <- plot_grid(plotlist = plts, ncol = 1, nrow = 2)

save_plot(file.path(f2_outdir, 
                    "tsne_naive_injured_by_sample_type",
                    "2B_tsne_cell_cycle_arrest_injured_only.pdf"), 
          plt,
          ncol = 1,
          nrow = 2,
          base_height = 3.75, 
...
p <- DotPlot(td_atcells, 
  ...
  labs(colour = "Z-Score\n(Average\nExpression)",
           size = "% of Cells\nwith Expression") +
  scale_y_discrete(limits = cell_label_order, labels = pretty_label_order) +
  theme(axis.text.y = element_text(face = "italic"),
        axis.text.x = element_text(hjust = 0.95,vjust = 0.2))
           
p

save_plot(file.path(f2_outdir, "2E_dotplot_cca_proliferation_markers.pdf"), 
          p, 
          base_width = 6, 
          base_height = 6)
```
...
rownames(atcells@meta.data)[atcells@meta.data$pretty_cell_labels == "Naive AEC1"]
...
save_plot(file.path(s8_outdir, "igfbp2_tsne.pdf"), plt,  base_aspect_ratio = 1.33)

plot_feature(atcells, "Igfbp2", 
             cell_filter = naive_injured_cell_types$Combined,
             pt_alpha = 1,
             pt_size = 1
) +
...
save_plot(file.path(s8_outdir, "igfbp2_tsne_smaller_dots.pdf"), plt,  base_aspect_ratio = 1.33)

```


```{r hmaps_td_pops_plus_injured_no_naive}
...
hmat <- mat[, c(td_cell_types, c("Injured Type II"))]
hmat <- t(scale(t(hmat)))

pdf(file.path(f3_outdir, "3A_td_injured_nonaive_top_30markers_viridiscols.pdf"), width = 24, height = 6)
...
dir.create(file.path(f4_outdir, "tsne_plots"), 
...
walk2(plts, 
      tgf_markers$tgfbeta,
      ~save_plot(filename = file.path(f4_outdir, "tsne_plots",
                                     paste0(.y, ".pdf")),
                .x,
                base_aspect_ratio = 1.33))
...
s9_outdir <- file.path(main_outdir, "supplement", "fig9")
dir.create(s9_outdir, showWarnings = F)
...
tgf_cell_types <- all_cell_types[str_detect(all_cell_types, 
                               "Naive Type II|Proliferating|Transdifferentiating|Arrest|Injured")]
...
cell_pretty_labels <- c(
  "Naive AEC2",
  "Other Injured AEC2-Derived",
  "Proliferating",
  "Cell Cycle Arrest",
  "Transdifferentiating"
)
...
p <- DotPlot(tgf_atcells, 
  ...
  scale_y_discrete(limits = cell_label_order, labels = cell_pretty_labels) +
  theme(axis.text.x = element_text(hjust = 0.95,vjust = 0.2),
        axis.text.y = element_text(face = "italic"))
           
p

save_plot(file.path(f4_outdir, "4B_tgfb_markers_dotplot.pdf"), 
          p, 
          base_width = 6, 
          base_height = 6)
```
...
```{r hmaps_td_pops_plus_injured, eval = FALSE}
...
hmat <- mat[, c(td_cell_types, c("Injured Type II", "Naive Type II"))]
hmat <- t(scale(t(hmat)))

pdf(file.path(f4_outdir, "td_injured_top_30markers_viridiscols.pdf"), width = 24, height = 6)
Heatmap(t(hmat), col = viridis(9),
        name = "Expression\n(Z-score)",
        cluster_rows = F,
        cluster_columns = F,
        column_order = gene_order,
...
hmat <- mat[, c(td_cell_types, c( "Naive Type I",  "Naive Type II", "Injured Type II"))]
hmat <- t(scale(t(hmat)))

pdf(file.path(f4_outdir, "td_injured_plus_naive_top_30markers_viridiscols.pdf"), width = 24, height = 6)
Heatmap(t(hmat), col = viridis(9),
        name = "Expression\n(Z-score)",
        cluster_rows = F,
        cluster_columns = F,
...
write_lines("Also included TD populations with the navive and injured heatmap with top 30 markers",
            file.path(f4_outdir, "README.txt"), append = T) 
...
```{r, eval = FALSE}
markers <- read_tsv(file.path("markers",
                          "td_pops_and_naive_markers_wilcox.txt"))
...
gene_order <- left_join(data_frame(cluster = alv_cell_types),
                        top_n_markers) %>% 
  pull(gene)

hmat <- mat[, alv_cell_types]
hmat <- t(scale(t(hmat)))

pdf(file.path(f4_outdir, "td_naive_top_30markers_viridiscols.pdf"), width = 30, height = 5)
Heatmap(t(hmat), 
        col = viridis(9),
        name = "Expression\n(Z-score)",
        cluster_rows = F,
...
write_lines("Also included TD populations with naive aec1 and aec2 heatmap with top 30 markers",
            file.path(f4_outdir, "README.txt"), append = T)
...
pdf(file.path(f4_outdir, "td_naive_aec2_only_top_30markers_viridiscols.pdf"), width = 30, height = 5)
Heatmap(t(hmat), 
        col = viridis(9),
        name = "Expression\n(Z-score)",
        cluster_rows = F,
...
write_lines("Also included TD populations with naive aec2 heatmap with top 30 markers",
            file.path(f4_outdir, "README.txt"), append = T)
...
save_plot(file.path(s8_outdir, "injured_aec2_cells_in_naive_aec2_cluster.pdf"),
          p, 
...
save_plot(file.path(s8_outdir, "injured_aec2_cells_in_naive_aec2_cluster_gfp.pdf"),
          p, 
...
save_plot(file.path(s8_outdir, "cells_per_cluster.pdf"), 
          plt, nrow = 3, ncol = 1, base_aspect_ratio = 3)

write_tsv(cluster_summary,
          file.path(s8_outdir, "cells_per_cluster_counts.txt"))



cluster_ids <- atcells@meta.data$res.0.6
cell_ids <- rownames(atcells@meta.data)
...
write_tsv(cluster_summary,
          file.path(s8_outdir, "cells_per_all_clusters_counts.txt"))

cca_cells <- rownames(atcells@meta.data)[atcells@meta.data$pretty_cell_labels == "Injured AEC2: Cell Cycle Arrest"]
cca_cells <- SubsetData(atcells, cells.use = cca_cells)
...
comps_to_do <- list(
  ...
  c("Tgfb2"),
  c("Itgb6"),
  c("Cdkn2b", "Itgb6", "Tgfb2")
)

names(comps_to_do) <- map(comps_to_do,
                          ~paste0("% of CCA cells that express ", paste0(.x, collapse = " ")))

ncells <- ncol(cca_cells@data)

percent_cca_out <- map(comps_to_do,
    ~apply(cca_cells@data[.x, , drop = FALSE], 2, function(x) all(x > 0)) %>% 
      sum(.) * 100 / ncells) %>% 
  bind_rows(., .id = "") %>% 
  t(.)

write.table(percent_cca_out, file.path(new_outdir, 
                                       "percent_positive_for_markers.tsv"))
```
...
pdf(file.path(s8_outdir, "heatmap_all_clusters.pdf"), 
    width = 10, height = 8)
...
pdf(file.path(s8_outdir, "heatmap_all_labeled_clusters.pdf"), 
    width = 10, height = 10)
...
### Heatmap from exhaustive list of fibroblast markers

```{r}

mes_genes <- readxl::read_excel(file.path("extdata",
                                          "Mesenchymal gene list.xlsx"),
                                col_names = FALSE)

colnames(mes_genes)[1] <-"gene"

atcells <- SetAllIdent(atcells, "pretty_cell_labels")

spec_cell_types <-
  c("Injured AEC2: Cell Cycle Arrest",
    "Injured AEC2: Transdifferentiating",
    "Injured AEC2: Proliferating", 
    "Endothelial/Fibroblast")

spec_cell_ids <- rownames(atcells@meta.data)[atcells@meta.data$pretty_cell_labels %in% spec_cell_types]
interesting_atcells <- SubsetData(atcells, cells.use = spec_cell_ids)


interesting_atcells <- ScaleData(interesting_atcells)

pdf(file.path(new_outdir, "heatmap_regenerating_clusters_mesenchymal_markers.pdf"), 
    width = 3, height = 6)

mouse_fibgenes <- intersect(rownames(interesting_atcells@scale.data),
                                  mes_genes$gene)

tmp_mat <- interesting_atcells@scale.data[mouse_fibgenes, ]

expr_genes <- mouse_fibgenes[rowSums(tmp_mat) != 0]
tmp_mat <- tmp_mat[rowSums(tmp_mat) != 0, ]

a <- hclust(dist(tmp_mat))

gene_order <- expr_genes[a$order]

DoHeatmap(interesting_atcells,
          genes.use = gene_order, 
          draw.line = T, 
          col.low = viridis(3)[1],
          col.mid = viridis(3)[2],
          col.high = viridis(3)[3],
...
symbol_con <- read_tsv(file.path("extdata",
"mart_export_mouse_to_human_ids.txt.gz"))

# from excel spreadsheet
tgf_beta <- readxl::read_excel("extdata/TGFb Genes.xlsx", 
                               col_names = FALSE)

fixed_genes <- tgf_beta[[1]] %>% 
  #str_remove(" (includes others)") %>% 
  str_remove_all(" \\(includes others\\)") %>% 
  str_replace("\\/", ",") %>% 
  str_split(",") %>% 
  .[[1]]

genes <- tibble(tgf_genes = fixed_genes)

cross_with_mouse <- inner_join(genes, 
                               symbol_con, 
                              by = c("tgf_genes" = "Gene name")) 

cross_with_human <- inner_join(genes, 
                               symbol_con, 
                              by = c("tgf_genes" = "Human gene name")) 

mouse_tgf_beta_genes <- c(cross_with_mouse$tgf_genes,
                          cross_with_human$`Gene name`) %>% 
  unique()
...
pdf(file.path(new_outdir, "heatmap_regenerating_clusters_tgfbeta_markers.pdf"), 
    width = 3, height = 8)

mouse_tgf_beta_genes <- intersect(rownames(interesting_atcells@scale.data),
                                  mouse_tgf_beta_genes)

tmp_mat <- interesting_atcells@scale.data[mouse_tgf_beta_genes, ]

expr_genes <- mouse_tgf_beta_genes[rowSums(tmp_mat) != 0]
tmp_mat <- tmp_mat[rowSums(tmp_mat) != 0, ]
...
sup1 <- read_excel(file.path("extdata", "lung_markers.xlsx"),
...
### GO terms for injured AEC2

COmpare all injured clusters to naive (in aggregare), extract

```{r}

david <- read_tsv(file.path("extdata", "go_analysis",
                   "david_injured_v_naiveaec2_bp.txt"))
...
david_log <- david %>% 
  ...
  mutate(Term =  str_replace(Term, "GO.*~", "")) %>% 
  dplyr::slice(1:10)


p <- ggplot(david_log, aes(x = reorder(Term, minus_log10), y = minus_log10)) + #reorder Term by minus_log10
...
save_plot(file.path(new_outdir, "go_terms_injured_v_naive.pdf"),
          p,
          base_asp = 2.5)
```
...
```{r}
markers_dir <- file.path("markers")
inj_v_cluster_markers <- read_tsv(file.path(markers_dir,
"injured_vs_naive_markers_wilcox.txt"))

atcells <- SetAllIdent(atcells, "res.0.6")

spec_cell_types <- as.character(c(0:5))

spec_cell_ids <- rownames(atcells@meta.data)[atcells@meta.data$res.0.6 %in% spec_cell_types]
inj_v_naive_atcells <- SubsetData(atcells, cells.use = spec_cell_ids)

top_genes <- filter(all_inj_v_cluster_markers, avg_logFC > 0) %>% 
  arrange(p_val_adj) %>% 
  slice(1:50)


inj_v_naive_atcells <- ScaleData(inj_v_naive_atcells)

pdf(file.path(new_outdir, "heatmap_injured_v_aec2_naive_clusters.pdf"), 
    width = 3, height = 4)

DoHeatmap(SetAllIdent(inj_v_naive_atcells, "res.0.6"),
          genes.use = top_genes$gene, 
          draw.line = T, 
          col.low = viridis(3)[1],
          col.mid = viridis(3)[2],
...
```
 ...
 plot a few more discriminating markers
```{r}
int_genes <- c(
"B2m",
"Ly6i",
...
save_plot(file.path(new_outdir, "injured_upregulatd_genes.pdf"), 
          plt, nrow = 2,
          ncol = 2,
          base_asp = 1.33)
...
idents_to_plot <- str_subset(colnames(atcells@meta.data),
                             "^res") %>% sort()

plts <- map(idents_to_plot[1:6],
    ~plot_feature(atcells, .x, label_text = TRUE,
                  label_color = "black") + 
      labs(title = paste0("resolution = ", str_remove(.x,
                                                      "res."))) + 
      theme(legend.pos = "none"))

p <- plot_grid(plotlist = plts, nrow = 2, ncol = 3)

save_plot(file.path(s4_outdir, "S4E_clustering_params.pdf"),
          p, nrow = 2, ncol = 2,
          base_asp = 1.33)
...
save_plot(file.path(s4_outdir, "S4E_clustering_params_no_titles.pdf"),
          p, nrow = 2, ncol = 2,
          base_asp = 1.33)
```
...
igf_dat <- FetchData(atcells, "Igfbp2") %>%
  as.data.frame() %>% 
  tibble::rownames_to_column("cell") 
...
igf_dat <- left_join(igf_dat, mdata, by = "cell")

igf_dat %>% 
  mutate(expressed = Igfbp2 > 0) %>% 
  group_by(pretty_cell_labels) %>%
  summarize(n = n(), n_expr = sum(expressed)) %>%
  mutate(percent_expr = 100 * n_expr / n) %>% 
  rename(cell_type = pretty_cell_labels, 
         n_cells_per_cluster = n, 
         n_igfbp2_pos_cells = n_expr,
         percent_igfbp2_pos_cells = percent_expr) %>% 
  openxlsx::write.xlsx("markers/percent_igfbp2_pos.xlsx")
```
...
mdata_out <- tibble::rownames_to_column(atcells@meta.data, 
                                        "cell") %>% 
...
tsne_coords <- atcells@dr$tsne@cell.embeddings %>% 
  as_tibble(rownames = "cell")

mdata_out <- left_join(mdata_out, tsne_coords, by = "cell")

write_tsv(mdata_out, 
          file.path(main_outdir, "geo", "cell_metadata.tsv"))
